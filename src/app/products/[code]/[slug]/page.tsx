                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
                    ...
                    <div className="lg:col-span-5 space-y-3 sticky top-32 h-fit">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <Link
                                href={`https://wa.me/62882005824231?text=Halo Atmosphere Furniture,%0A%0ASaya ingin berkonsultasi mengenai produk berikut:%0A%0A- *Produk*: ${product.name}%0A- *Kode*: ${product.code}%0A- *Kategori*: ${product.category}%0A- *Koleksi*: ${product.collection}%0A- *Status*: ${product.status}%0A- *Deskripsi*: ${(product.description || "").replace(/<[^>]*>?/gm, "").slice(0, 100)}...%0A- *Jumlah*: ${quantity} unit%0A- *Harga Normal*: ${formattedPrice}%0A- *Harga Promo*: ${formattedMember}%0A- *Total Estimasi*: Rp ${(product.memberPrice * quantity).toLocaleString("id-ID")}%0A%0A*Link Produk*: ${typeof window !== 'undefined' ? window.location.href : ''}`}
                                target="_blank"
                                className="h-12 border-2 border-zinc-900 text-zinc-900 text-base font-semibold tracking-tight hover:bg-zinc-900 hover:text-white transition-all rounded-xl cursor-pointer flex items-center justify-center"
                            >
                                Consult Now
                            </Link>
                        </div>
