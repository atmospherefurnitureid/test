export const translations = {
    ID: {
        navbar: {
            home: "Beranda",
            products: "Produk",
            services: "Layanan",
            artikel: "Artikel",
            contact: "Kontak",
            cart: "Keranjang",
            services_1: "Desain yang Dipersonalisasi Sepenuhnya",
            services_2: "Kualitas dan Ketelitian Pengerjaan",
            services_3: "Solusi Fungsional dan Estetis",
            services_4: "Kolaborasi untuk Hasil Terbaik",
            marquee: [
                "PRODUSEN FURNITURE BESI & KAYU BERKUALITAS EKSPOR",
                "MELAYANI CUSTOM DESIGN & PROYEK BESAR",
                "HARGA PABRIK & LEGALITAS LENGKAP"
            ]
        },
        home: {
            hero: {
                badge: "Jangkauan Global",
                badge_desc: "Bergabunglah dengan Daftar Klien Kami yang Terus Berkembang",
                title: "Furnitur Berkualitas Tinggi yang Bisa Dikustomisasi\nSesuai Keinginan untuk Mewujudkan Ide Anda",
                subtitle: "Sejak 2015, kami telah melayani berbagai negara dan membangun kepercayaan melalui seni alami. Atmosphere Furniture Indonesia - Mewujudkan Ide Furnitur Anda.",
                cta_primary: "Mulai Sekarang — Hubungi Kami!",
                cta_secondary: "Jelajahi Produk Kami"
            },
            about: {
                title: "Tentang Kami - Atmosphere Furniture Indonesia",
                desc: "Atmosphere Furniture Indonesia adalah perusahaan furnitur yang mengkhususkan diri dalam menciptakan potongan furnitur yang unik dan inovatif. Kami percaya bahwa furnitur harus menjadi cerminan dari orang yang menggunakannya.",
                headline: "Menjembatani Tradisi & Inovasi",
                brief: "Atmosphere Furniture Indonesia adalah produsen utama yang mengkhususkan diri dalam furnitur perpaduan kayu dan besi berkualitas tinggi. Sejak 2015, kami telah berdedikasi untuk menjembatani pengerjaan tradisional dengan presisi desain modern.",
                vision_title: "Visi Kami",
                vision_desc: "Menjadi inspirasi global terkemuka dalam mewujudkan ide furnitur yang inovatif dan artistik yang membawa jiwa ke setiap ruang.",
                mission_title: "Misi Kami",
                mission_desc: "Memberikan kualitas unggul, memupuk kepercayaan melalui seni yang konsisten, dan melayani kebutuhan furnitur global.",
                stats: {
                    projects: "Proyek Selesai",
                    team: "Anggota Tim",
                    reviews: "Ulasan Klien",
                    awards: "Penghargaan"
                }
            },
            services: {
                title: "Layanan yang Disesuaikan untuk Anda",
                subtitle: "Kami menyediakan solusi furnitur dan interior kustom yang dirancang untuk memenuhi kebutuhan unik Anda, memastikan setiap bagian mencerminkan visi Anda dan melampaui ekspektasi.",
                card1: {
                    title: "Desain yang Dipersonalisasi Sepenuhnya",
                    desc: "Kami menyediakan furnitur dan solusi interior yang dirancang khusus sesuai kebutuhan, gaya, dan preferensi Anda. Setiap detail dibuat untuk mencerminkan visi pribadi Anda."
                },
                card2: {
                    title: "Kualitas dan Ketelitian Pengerjaan",
                    desc: "Setiap produk dibuat dengan material pilihan dan proses pengerjaan yang teliti. Kami memastikan setiap sudut dan finishing memenuhi standar kualitas tinggi."
                },
                card3: {
                    title: "Solusi Fungsional dan Estetis",
                    desc: "Kami menggabungkan fungsi dan estetika dalam setiap desain, memastikan setiap furnitur memberikan kenyamanan serta efisiensi ruang yang optimal."
                },
                card4: {
                    title: "Kolaborasi untuk Hasil Terbaik",
                    desc: "Kami bekerja sama dengan Anda dari tahap konsep hingga pemasangan akhir dengan komunikasi terbuka untuk memastikan hasil melampaui ekspektasi."
                }
            },
            team: {
                title: "Kami memiliki seluruh tim untuk mendukung bisnis",
                ceo_desc: "Chief Executive Officer (CEO) & Pendiri Atmosphere Furniture Indonesia.",
                quote: "\"Membuat furnitur bukan hanya tentang menyambung kayu dan besi; ini tentang menenun cerita ke dalam setiap serat kayu, menciptakan harmoni antara bentuk dan fungsi yang bertahan lama.\"",
                master: "Pendiri & Pengrajin Utama"
            },
            articles: {
                title: "Wawasan Terbaru & Berita Atmosphere",
                desc: "Jelajahi berita industri terbaru, wawancara, dan sumber daya untuk membantu Anda menciptakan suasana yang sempurna bagi ruang tamu premium Anda."
            },
            cta: {
                tagline: "Produsen Furnitur Besi & Kayu Custom Kualitas Ekspor",
                description: "Wujudkan ide furnitur Anda bersama pengrajin berpengalaman Jepara.",
                quote: "\"Hilangkan keraguan Anda dan mulailah melangkah untuk mewujudkan ide furnitur Anda\""
            }
        },
        articles: {
            title: "Cerita & Wawasan Atmosphere",
            subtitle: "Berita industri terbaru, wawancara, dan sumber daya untuk membantu Anda menciptakan suasana yang sempurna bagi ruang tamu premium Anda.",
            category: "Kategori",
            stats: "artikel",
            search_placeholder: "Cari Judul atau Kategori Artikel...",
            search_hint: "Ketik {count} karakter lagi untuk mencari...",
            no_results: "Tidak ada artikel ditemukan dalam kategori ini.",
            read_time: "5 menit baca"
        },
        contact: {
            title: "Kami antusias menyambut Anda 24/7!",
            subtitle: "Tim kami selalu siap membantu mewujudkan ide furnitur Anda. Hubungi kami dan kami akan membalas dalam waktu 24 jam.",
            form: {
                name: "Nama",
                name_placeholder: "Nama lengkap Anda",
                email: "Email",
                email_placeholder: "Alamat email",
                whatsapp: "WhatsApp",
                whatsapp_placeholder: "misal. +62 812...",
                address: "Alamat",
                address_placeholder: "Masukkan alamat Anda",
                needs: "Kebutuhan",
                needs_placeholder: "Pilih Kebutuhan...",
                needs_options: {
                    consultation: "Konsultasi",
                    order: "Pesan Produk"
                },
                message: "Pesan",
                message_placeholder: "Tinggalkan pesan untuk kami...",
                privacy_policy: "Anda menyetujui kebijakan privasi kami yang ramah.",
                submit: "Kirim pesan"
            },
            details: {
                tagline: "Besi berkualitas tinggi sejak 2015.",
                quote: "\"Hilangkan keraguan Anda dan mulailah melangkah untuk mewujudkan ide furnitur Anda\"",
                founder: "Founder",
                payment_methods: "Metode Pembayaran Global"
            },
            steps: {
                title: "Langkah-Langkah Pemesanan",
                subtitle: "Proses kami yang efisien memastikan perjalanan furnitur kustom Anda berjalan lancar, transparan, dan memenuhi standar kualitas tertinggi di setiap tahap.",
                step1: {
                    title: "Konsultasi",
                    desc: "Diskusikan ide dan kebutuhan furnitur Anda bersama tim ahli kami secara gratis untuk mendapatkan konsep terbaik."
                },
                step2: {
                    title: "DP 50%",
                    desc: "Pembayaran uang muka sebesar 50% untuk mengamankan pesanan Anda dan memulai pengadaan material."
                },
                step3: {
                    title: "Pengerjaan",
                    desc: "Proses produksi dilakukan oleh pengrajin berpengalaman Jepara dengan pengawasan kualitas yang ketat."
                },
                step4: {
                    title: "Pelunasan",
                    desc: "Setelah produk selesai dan dikonfirmasi melalui foto/video, pembayaran sisa dapat dilakukan sebelum pengiriman."
                },
                step5: {
                    title: "Pengiriman",
                    desc: "Produk Anda dikirim menggunakan ekspedisi terpercaya dengan packing yang aman sampai ke tujuan."
                },
                step6: {
                    title: "Garansi",
                    desc: "Kami menjamin kualitas produk kami. Jika terjadi kerusakan selama pengiriman, segera hubungi kami."
                }
            }
        },
        footer: {
            rights: "Semua hak dilindungi.",
            privacy: "Kebijakan Privasi",
            terms: "Syarat & Ketentuan",
            cookies: "Cookies"
        },
        services: {
            ui: {
                about_service: "Tentang Layanan",
                why_choose: "Mengapa Memilih Layanan Ini?",
                start_project: "Mulai Proyek Anda",
                contact_desc: "Tertarik dengan layanan kami? Hubungi tim kami untuk konsultasi gratis dan penawaran khusus.",
                free_consultation: "Konsultasi Gratis",
                other_services: "Layanan Lainnya",
                back_link: "Kembali ke Semua Layanan",
                not_found: "Layanan tidak ditemukan",
                back_home: "Kembali ke Beranda"
            },
            items: {
                "desain-yang-dipersonalisasi-sepenuhnya": {
                    title: "Desain yang Dipersonalisasi Sepenuhnya",
                    description: "Kami menyediakan furnitur dan solusi interior yang dirancang khusus sesuai kebutuhan, gaya, dan preferensi Anda. Setiap detail dibuat untuk mencerminkan visi pribadi Anda.",
                    longDescription: "Layanan desain personalisasi kami memungkinkan Anda untuk menciptakan furnitur yang benar-benar unik dan sesuai dengan karakter ruangan Anda. Tim desainer profesional kami akan bekerja sama dengan Anda mulai dari konsep awal hingga eksekusi akhir, memastikan setiap detail mencerminkan kepribadian dan gaya hidup Anda.",
                    features: [
                        "Konsultasi desain gratis",
                        "Material pilihan berkualitas tinggi",
                        "Proses pengerjaan yang teliti",
                        "Garansi kepuasan 100%",
                        "Tim profesional berpengalaman"
                    ]
                },
                "kualitas-dan-ketelitian-pengerjaan": {
                    title: "Kualitas dan Ketelitian Pengerjaan",
                    description: "Setiap produk dibuat dengan material pilihan dan proses pengerjaan yang teliti. Kami memastikan setiap sudut dan finishing memenuhi standar kualitas tinggi.",
                    longDescription: "Kualitas adalah prioritas utama kami. Setiap furnitur yang kami buat melalui proses kontrol kualitas yang ketat, mulai dari pemilihan material, proses produksi, hingga finishing akhir. Kami menggunakan teknologi modern dan teknik tradisional untuk memastikan produk yang tahan lama dan estetis.",
                    features: [
                        "Material kayu pilihan grade A",
                        "Finishing halus dan tahan lama",
                        "Konstruksi kokoh dan stabil",
                        "Detail craftsmanship yang presisi",
                        "Standar kualitas internasional"
                    ]
                },
                "solusi-fungsional-dan-estetis": {
                    title: "Solusi Fungsional dan Estetis",
                    description: "Kami menggabungkan fungsi dan estetika dalam setiap desain, memastikan setiap furnitur memberikan kenyamanan serta efisiensi ruang yang optimal.",
                    longDescription: "Kami memahami bahwa furnitur yang baik tidak hanya tentang tampilan, tetapi juga tentang fungsi. Tim desainer kami menciptakan solusi yang mengoptimalkan ruang, meningkatkan kenyamanan, dan memperindah interior Anda secara bersamaan.",
                    features: [
                        "Desain ergonomis untuk kenyamanan",
                        "Optimasi ruang yang maksimal",
                        "Kombinasi estetika dan fungsi",
                        "Solusi penyimpanan kreatif",
                        "Adaptif untuk berbagai kebutuhan"
                    ]
                },
                "kolaborasi-untuk-hasil-terbaik": {
                    title: "Kolaborasi untuk Hasil Terbaik",
                    description: "Kami bekerja sama dengan Anda dari tahap konsep hingga pemasangan akhir dengan komunikasi terbuka untuk memastikan hasil melampaui ekspektasi.",
                    longDescription: "Kolaborasi adalah kunci kesuksesan proyek furnitur Anda. Kami melibatkan Anda dalam setiap tahap proses, dari brainstorming ide, review desain, hingga implementasi. Pendekatan kolaboratif ini memastikan hasil akhir sesuai dengan visi Anda.",
                    features: [
                        "Konsultasi berkelanjutan",
                        "Update progress reguler",
                        "Flexibilitas dalam perubahan desain",
                        "Tim support yang responsif",
                        "Komitmen terhadap kepuasan klien"
                    ]
                }
            }
        },
        contact_thanks: "terima kasih telah mengunjungi kami :)"
    },
    EN: {
        navbar: {
            home: "Home",
            products: "Products",
            services: "Services",
            artikel: "Articles",
            contact: "Contact",
            cart: "Cart",
            services_1: "Fully Personalized Design",
            services_2: "Quality and Craftsmanship Precision",
            services_3: "Functional and Aesthetic Solutions",
            services_4: "Collaboration for Best Results",
            marquee: [
                "EXPORT QUALITY IRON & WOOD FURNITURE MANUFACTURER",
                "SERVING CUSTOM DESIGN & LARGE PROJECTS",
                "FACTORY PRICE & COMPLETE LEGALITY"
            ]
        },
        home: {
            hero: {
                badge: "Global Reach",
                badge_desc: "Join Our Growing List of Clients",
                title: "High-Quality Customizable Furniture\nTailored to Bring Your Ideas to Life",
                subtitle: "Since 2015, we've been serving various countries and building trust through natural art. Atmosphere Furniture Indonesia - Realizing Your Furniture Idea.",
                cta_primary: "Get Started — Contact Us!",
                cta_secondary: "Explore Our Products"
            },
            about: {
                title: "About Us - Atmosphere Furniture Indonesia",
                desc: "Atmosphere Furniture Indonesia is a furniture company that specializes in creating unique and innovative furniture pieces. We believe that furniture should be a reflection of the people who use it.",
                headline: "Bridging Tradition & Innovation",
                brief: "Atmosphere Furniture Indonesia is a premier manufacturer specializing in high-quality wood and iron fusion furniture. Since 2015, we have been dedicated to bridging traditional craftsmanship with modern design precision.",
                vision_title: "Our Vision",
                vision_desc: "To be the leading global inspiration in realizing innovative and artistic furniture ideas that bring soul to every space.",
                mission_title: "Our Mission",
                mission_desc: "Delivering superior quality, fostering trust through consistent artistry, and serving global furniture needs.",
                stats: {
                    projects: "Complete Projects",
                    team: "Team Members",
                    reviews: "Client Reviews",
                    awards: "Winning Awards"
                }
            },
            services: {
                title: "Services Tailored to You",
                subtitle: "We provide customized furniture and interior solutions designed to meet your unique needs, ensuring every piece reflects your vision and exceeds expectations.",
                card1: {
                    title: "Fully Personalized Design",
                    desc: "We provide customized furniture and interior solutions designed to meet your unique needs, styles, and preferences. Every detail is made to reflect your personal vision."
                },
                card2: {
                    title: "Quality and Craftsmanship Precision",
                    desc: "Every product is made with selected materials and a careful crafting process. We ensure every corner and finishing meets high quality standards."
                },
                card3: {
                    title: "Functional and Aesthetic Solutions",
                    desc: "We combine function and aesthetics in every design, ensuring every piece of furniture provides comfort and optimal space efficiency."
                },
                card4: {
                    title: "Collaboration for Best Results",
                    desc: "We work with you from the concept stage to the final installation with open communication to ensure results exceed expectations."
                }
            },
            team: {
                title: "We have got entire team to support your business",
                ceo_desc: "Chief Executive Officer (CEO) & Founder of Atmosphere Furniture Indonesia.",
                quote: "\"Crafting furniture is not just about joining wood and iron; it's about weaving stories into every grain, creating harmony between form and function that stands the test of time.\"",
                master: "Founder & Master Craftsman"
            },
            articles: {
                title: "Latest Insights & Atmosphere News",
                desc: "Explore the latest industry news, interviews, and resources to help you create the perfect atmosphere for your premium living spaces."
            },
            cta: {
                tagline: "Custom Wood & Iron Furniture Manufacturer of Export Quality",
                description: "Realize your furniture ideas with experienced craftsmen from Jepara.",
                quote: "\"Remove your doubts and start taking steps to realize your furniture idea\""
            }
        },
        articles: {
            title: "Atmosphere Stories & Insights",
            subtitle: "The latest industry news, interviews, and resources to help you create the perfect atmosphere for your premium living spaces.",
            category: "Category",
            stats: "articles",
            search_placeholder: "Search Article Title or Category...",
            search_hint: "Type {count} more characters to search...",
            no_results: "No articles found in this category.",
            read_time: "5 min read"
        },
        contact: {
            title: "We'd love to welcome you 24/7!",
            subtitle: "Our team is always ready to help your furniture idea becomes reality. Reach out and we'll get in touch within 24 hours.",
            form: {
                name: "Name",
                name_placeholder: "Your full name",
                email: "Email",
                email_placeholder: "Email address",
                whatsapp: "WhatsApp",
                whatsapp_placeholder: "e.g. +62 812...",
                address: "Address",
                address_placeholder: "Enter your address",
                needs: "Needs",
                needs_placeholder: "Select Needs...",
                needs_options: {
                    consultation: "Consultation",
                    order: "Order Product"
                },
                message: "Message",
                message_placeholder: "Leave us a message...",
                privacy_policy: "You agree to our friendly privacy policy.",
                submit: "Send message"
            },
            details: {
                tagline: "High-quality iron furniture since 2015.",
                quote: "\"Remove your doubts and start taking steps to realize your furniture idea\"",
                founder: "Founder",
                payment_methods: "Global Payment Methods"
            },
            steps: {
                title: "Steps-to-Steps Order",
                subtitle: "Our streamlined process ensures your custom furniture journey is smooth, transparent, and meets the highest standards of quality at every stage.",
                step1: {
                    title: "Consultation",
                    desc: "Discuss your furniture ideas and needs with our expert team for free to get the best concept."
                },
                step2: {
                    title: "DP 50%",
                    desc: "Down-payment of 50% to secure your order and begin material procurement."
                },
                step3: {
                    title: "Production",
                    desc: "The production process is carried out by experienced Jepara craftsmen with strict quality control."
                },
                step4: {
                    title: "Final Payment",
                    desc: "Once products are finished and confirmed via photo/video, remaining payment can be made before shipping."
                },
                step5: {
                    title: "Shipping",
                    desc: "Your products are shipped using trusted expeditions with secure packing to their destination."
                },
                step6: {
                    title: "Warranty",
                    desc: "We guarantee the quality of our products. If damage occurs during shipping, contact us immediately."
                }
            }
        },
        footer: {
            rights: "All rights reserved.",
            privacy: "Privacy Policy",
            terms: "Terms of Service",
            cookies: "Cookies"
        },
        services: {
            ui: {
                about_service: "About Service",
                why_choose: "Why Choose This Service?",
                start_project: "Start Your Project",
                contact_desc: "Interested in our services? Contact our team for a free consultation and special offer.",
                free_consultation: "Free Consultation",
                other_services: "Other Services",
                back_link: "Back to All Services",
                not_found: "Service not found",
                back_home: "Back to Home"
            },
            items: {
                "desain-yang-dipersonalisasi-sepenuhnya": {
                    title: "Fully Personalized Design",
                    description: "We provide customized furniture and interior solutions designed to meet your unique needs, styles, and preferences. Every detail is made to reflect your personal vision.",
                    longDescription: "Our personalized design service allows you to create furniture that is truly unique and matches the character of your space. Our professional design team will work with you from the initial concept to the final execution, ensuring every detail reflects your personality and lifestyle.",
                    features: [
                        "Free design consultation",
                        "High-quality selected materials",
                        "Meticulous crafting process",
                        "100% satisfaction guarantee",
                        "Experienced professional team"
                    ]
                },
                "kualitas-dan-ketelitian-pengerjaan": {
                    title: "Quality and Craftsmanship Precision",
                    description: "Every product is made with selected materials and a careful crafting process. We ensure every corner and finishing meets high quality standards.",
                    longDescription: "Quality is our top priority. Every piece of furniture we create undergoes a strict quality control process, from material selection, production process, to final finishing. We use modern technology and traditional techniques to ensure durable and aesthetic products.",
                    features: [
                        "Grade A selected wood materials",
                        "Smooth and durable finishing",
                        "Sturdy and stable construction",
                        "Precise craftsmanship details",
                        "International quality standards"
                    ]
                },
                "solusi-fungsional-dan-estetis": {
                    title: "Functional and Aesthetic Solutions",
                    description: "We combine function and aesthetics in every design, ensuring every piece of furniture provides comfort and optimal space efficiency.",
                    longDescription: "We understand that good furniture is not just about looks, but also about function. Our design team creates solutions that optimize space, increase comfort, and beautify your interior simultaneously.",
                    features: [
                        "Ergonomic design for comfort",
                        "Maximum space optimization",
                        "Combination of aesthetics and function",
                        "Creative storage solutions",
                        "Adaptive for various needs"
                    ]
                },
                "kolaborasi-untuk-hasil-terbaik": {
                    title: "Collaboration for Best Results",
                    description: "We work with you from the concept stage to the final installation with open communication to ensure results exceed expectations.",
                    longDescription: "Collaboration is the key to the success of your furniture project. We involve you in every stage of the process, from brainstorming ideas, design reviews, to implementation. This collaborative approach ensures the final result matches your vision.",
                    features: [
                        "Continuous consultation",
                        "Regular progress updates",
                        "Flexibility in design changes",
                        "Responsive support team",
                        "Commitment to client satisfaction"
                    ]
                }
            }
        },
        contact_thanks: "thanks for visiting us :)"
    }
};
